...
//log that the module is loaded
    console.log(` :: ⬜️ Modulo: ${description.name} | caricato con versione ${description.version} da ("${description.filename}")`)
  ...
  if (oldState.channelID && !newState.channelID) {
          ...
          if (jointocreatemap.get(`tempvoicechannel_${oldState.guild.id}_${oldState.channelID}`)) {
            ...
            if (vc.members.size < 1) { 
              ...
              client.channels.cache.get('985622404304879676').send(
                new MessageEmbed()
                .setTitle('STANZA PRIVATA')
                .setColor('RED')
                .setDescription(`<@${oldState.member.user.id}> è uscito dal canale e la sua stanza privata è stata eliminata!`));
  ...
  if (oldState.channelID && newState.channelID) {
    ...
    async function jointocreatechannel(user) {
      ...
      client.channels.cache.get('985622404304879676').send(
        ...
        .setTitle('STANZA PRIVATA')
        .setColor('GREEN')
        .setDescription(`<@${user.member.user.id}> Ha creato la sua stanza privata!`));
      ...
      await user.guild.channels.create(`${user.member.user.username}'s Room`, {
        type: 'voice',
        parent: '985549799627554846', //or set it as a category id
      }).then(async vc => {
        ...
        await vc.overwritePermissions([
          {
            id: user.id,
            allow: ['MANAGE_CHANNELS', 'CONNECT', 'MUTE_MEMBERS', 'DEAFEN_MEMBERS'],
          },
          {
            id: user.guild.id,
